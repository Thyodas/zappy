...
GUI::Vector2f GUI::RayLib::getMousePosition() { return _mousePosition; }

void GUI::RayLib::loadModels(
    std::unordered_map<ModelEntity, modelConfig> models)
{
    _models.reserve(models.size());
    for (auto &model : models) {
        if (model.second.modelPath == "skybox") {
            ModelData skymodel;
            Mesh      cube = GenMeshCube(1.0f * model.second.scale,
                                         1.0f * model.second.scale,
                                         1.0f * model.second.scale);
            skymodel.model = LoadModelFromMesh(cube);
            Image img      = LoadImage(model.second.texturePath.c_str());
            skymodel.model.materials[0].maps[MATERIAL_MAP_CUBEMAP].texture =
                LoadTextureCubemap(img, CUBEMAP_LAYOUT_AUTO_DETECT);
            UnloadImage(img);
            Texture2D texture = LoadTexture(model.second.texturePath.c_str());
            SetMaterialTexture(&skymodel.model.materials[0],
                               MATERIAL_MAP_DIFFUSE, texture);
            _models.insert({ModelEntity::SKYBOX, skymodel});
            continue;
        }
        ...
        if (model.second.animation) {
            ...
            _models[model.first].animation =
                LoadModelAnimations(model.second.modelPath.c_str(), &models);
        }
        if (model.second.modelPath.length() > 4 &&
            model.second.modelPath.substr(model.second.modelPath.length() -
                                          4) != ".glb") {
            _models[model.first].texture =
                LoadTexture(model.second.texturePath.c_str());
            _models[model.first]
                .model.materials[0]
                .maps[MATERIAL_MAP_DIFFUSE]
                .texture = _models[model.first].texture;
        }
        BoundingBox bounds = GetModelBoundingBox(_models[model.first].model);
        _models[model.first].size = {bounds.max.x - bounds.min.x,
                                     bounds.max.y - bounds.min.y,
                                     bounds.max.z - bounds.min.z};
    }
}

void GUI::RayLib::drawModel(ModelEntity model, Vector3f position, float scale,
                            Vector3f rotation)
{
    DrawModelEx(_models[model].model, {position.x, position.y, position.z},
                {rotation.x, rotation.y, rotation.z}, rotation.x,
...
void GUI::RayLib::postDraw() { EndDrawing(); }
...
void GUI::RayLib::clear(GUI::C_Color color) { ClearBackground(_colors[color]); }
...
void GUI::RayLib::drawGrid(Vector2i size, float spacing, GUI::C_Color color)
{
    ...
    for (int x = startX; x <= endX; x += spacing) {
        DrawLine3D(Vector3{(float)x, 0.0f, startY},
                   Vector3{(float)x, 0.0f, endY}, _colors[color]);
    }
    for (int y = startY; y <= endY; y += spacing) {
        DrawLine3D(Vector3{startX, 0.0f, (float)y},
                   Vector3{endX, 0.0f, (float)y}, _colors[color]);
    }
}

GUI::Vector3f GUI::RayLib::mousePosFromGrid(GUI::Vector2i position,
                                            int           cellSize,
                                            GUI::Vector2i numberOfCells)
{
...
bool GUI::RayLib::isMouseButtonPressed(GUI::Mouse button)
{
    for (auto &pressedMouseButton : _pressedMouseButtons) {
        if (pressedMouseButton.first == button &&
            pressedMouseButton.second == true) {
...
void GUI::RayLib::drawText(std::string text, GUI::Vector2f position,
                           int fontSize, GUI::C_Color color)
{
    DrawText(text.c_str(), position.x, position.y, fontSize, _colors[color]);
}

void GUI::RayLib::drawRectangle(GUI::Vector2f position, GUI::Vector2f size,
                                GUI::C_Color color)
{
...
void GUI::RayLib::disable3DMode() { EndMode3D(); }

bool GUI::RayLib::isInteraction()
{
    return _pressedKeys.size() > 0 || _pressedMouseButtons.size() > 0 ||
           _pressedMouseButtons.size() > 0;
}

void GUI::RayLib::animateModel(ModelEntity model, AnimationType type, int frame)
{
    UpdateModelAnimation(_models[model].model, _models[model].animation[type],
                         frame);
}
...
void GUI::RayLib::rotateModel(ModelEntity model, Direction direction)
{
    switch (direction) {
        case Direction::NORTH:
            _models[model].model.transform =
                MatrixRotateXYZ({0, 0, DEGREES_RADIAN(180)});
            break;
        case Direction::EAST:
            _models[model].model.transform =
                MatrixRotateXYZ({0, 0, DEGREES_RADIAN(270)});
            break;
        case Direction::SOUTH:
            _models[model].model.transform =
                MatrixRotateXYZ({0, 0, DEGREES_RADIAN(0)});
            break;
        case Direction::WEST:
            _models[model].model.transform =
                MatrixRotateXYZ({0, 0, DEGREES_RADIAN(90)});
            break;
        default:
            break;
    }
...
bool GUI::RayLib::isModelSelected(ModelEntity model, Vector3f position,
                                  float scale, std::shared_ptr<ICamera> camera)
{
    _ray       = GetMouseRay(GetMousePosition(),
                             std::dynamic_pointer_cast<RayLibCamera>(camera)->camera);
    _collision = GetRayCollisionBox(
        _ray,
        (BoundingBox){
            (Vector3){position.x - (this->getModelSize(model).x * scale) / 2,
                      position.y - (this->getModelSize(model).y * scale) / 2,
                      position.z - (this->getModelSize(model).z * scale) / 2},
            (Vector3){position.x + (this->getModelSize(model).x * scale) / 2,
                      position.y + (this->getModelSize(model).y * scale) / 2,
                      position.z + (this->getModelSize(model).z * scale) / 2}
    });
...
void GUI::RayLib::disableFaceCulling()
{
    rlDisableBackfaceCulling();
    rlDisableDepthMask();
}

void GUI::RayLib::enableFaceCulling()
{
