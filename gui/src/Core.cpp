      ...
      _teamSelection(false)
{
    ...
    _models = {
        GUI::ModelEntity::FOOD,      GUI::ModelEntity::LINEMATE,
        GUI::ModelEntity::DERAUMERE, GUI::ModelEntity::SIBUR,
        GUI::ModelEntity::MENDIANE,  GUI::ModelEntity::PHIRAS,
...
void GUI::Core::init(GUI::GraphicalLib lib, GUI::Vector2i windowSize)
{
    ...
    _coms.getConf()->getActions().setGridSize(
        _module->getModelSize(ModelEntity::GRASS_BLOCK));

    while (!this->_coms.getConf()->isInitialized()) this->_coms.process();
    _map = std::make_shared<Map>(_coms.getConf()->getMapSize(),
                                 _coms.getConf()->getMapContent());
    ...
    _scene->getCamera()->setPosition(
        Vector3f(_map->getSize().x * groundSize.x / 2, 20, 20));
}
...
void GUI::Core::handleEndGame()
{
    std::string endMessage =
        "The winner is: " + _coms.getConf()->getWinnerTeam();
    ...
    while (!_module->isKeyPressed(GUI::Key::ESCAPE)) {
        ...
        _module->drawText(
            endMessage,
            (Vector2f){static_cast<float>(
                           (_windowSize.x - endMessage.length() * 25 / 2) / 2),
                       static_cast<float>(_windowSize.y / 2)},
            25, GUI::C_Color::C_GREEN);
        _module->drawText(
            exitMsg,
            (Vector2f){static_cast<float>(
                           (_windowSize.x - exitMsg.length() * 25 / 2) / 2),
                       static_cast<float>(_windowSize.y / 2 + 50)},
            25, GUI::C_Color::C_GREEN);
        _module->postDraw();
    }
}
...
void GUI::Core::drawGround()
{
    ...
    for (int y  = -(_map->getSize().y * grassSize.x / 2 - (grassSize.x / 2)),
             yy = 0;
         y <= (_map->getSize().y * grassSize.x / 2 - (grassSize.x / 2));
         y += 2, yy++) {
        for (int x = -(_map->getSize().x * grassSize.z / 2 - (grassSize.z / 2)),
                 xx = 0;
             x <= (_map->getSize().x * grassSize.z / 2 - (grassSize.z / 2));
             x += 2, xx++) {
            if (_map->selectionMode() &&
                _map->selectionType() == SelectionType::BLOCK &&
                _map->getSelectionBlock() == (GUI::Vector2i){xx, yy})
                continue;
            else
                _module->drawModel(ModelEntity::GRASS_BLOCK,
                                   Vector3f(x, -grassSize.y, y), 1,
                                   Vector3f(0, 0, 0));
        }
    }
}

void GUI::Core::drawSkybox()
{
    _module->disableFaceCulling();
    _module->drawModelCube(ModelEntity::SKYBOX, Vector3f(0, 0, 0), 100.0f);
    _module->enableFaceCulling();
}
...
void GUI::Core::draw()
{
    ...
    this->drawSkybox();
    this->drawGround();
    _module->drawGrid(_map->getSize(),
                      _module->getModelSize(ModelEntity::GRASS_BLOCK).x,
                      GUI::C_Color::C_RED);
    if (_drawObjects) {
...
void GUI::Core::drawEggs()
{
    ...
    for (auto &egg : _coms.getConf()->getEggs()) {
        Vector3f pos = _module->mousePosFromGrid(egg.second.pos, grassSize.x,
                                                 _map->getSize());
        pos.x += grassSize.x - (eggSize.x * _config.models[EGG].scale) / 2;
        pos.z += grassSize.z - (eggSize.z * _config.models[EGG].scale) / 2;
        _module->drawModel(ModelEntity::EGG, pos, _config.models[EGG].scale,
                           {0, 0, 0});
    }
}
...
void GUI::Core::drawCellDetails(std::shared_ptr<ICell> cell)
{
    ...
    if (_map->selectionType() == SelectionType::BLOCK) {
        ...
        if (_coms.getConf()->getPlayers().find(_map->getPlayerId()) ==
            _coms.getConf()->getPlayers().end())
            return;
        std::shared_ptr<IPlayer> player =
            _coms.getConf()->getPlayers()[_map->getPlayerId()];
    ...
    _module->drawRectangle(
        (Vector2f){0, 0},
        (Vector2f){static_cast<float>(30 * _windowSize.x / 100),
    ...
    for (auto &i : _objectsMap) {
        _module->drawText(
            ...
            (Vector2f){10,
                       static_cast<float>(
                           _windowSize.y / _objectsMap.size() * index + 40)},
            20, GUI::C_Color::C_WHITE);
        _module->drawText(
            std::to_string(stock[i.first]),
            (Vector2f){200,
                       static_cast<float>(
                           _windowSize.y / _objectsMap.size() * index + 40)},
            20, GUI::C_Color::C_WHITE);
...
void GUI::Core::drawEntities(std::shared_ptr<ICell> cell)
{
    ...
    for (auto &obj : cell->getObjects()) {
        if (obj.second >= 1) {
            Vector3f pos = _module->mousePosFromGrid(
                cell->getPos(), grassSize.x, _map->getSize());
            ...
            if (model != ModelEntity::FOOD) pos.y += modelSize.y / 4;
            _module->drawModel(_objToModels[obj.first], pos,
                               _config.models[model].scale, Vector3f(0, 0, 0));
            offsetX += 0.25;
            count++;
        }
    }
...
void GUI::Core::drawPlayers()
{
    ...
    for (auto &player : _coms.getConf()->getPlayers()) {
        if (player.second == nullptr ||
            (player.second != nullptr && player.second->getIsAlive() == false))
            continue;
        Vector3f pos = _module->mousePosFromGrid(
        ...
        if (actions.isAction(player.first)) {
            ...
            AnimationType animation = actions.execute(
                player.second, _map->getCell(player.second->getPos()), action,
                _coms.getConf()->getPlayers(),
                _coms.getConf()->getClock()->getElapsedTime());
            ...
            float framerate = static_cast<float>(
                _module->getMaxFrame(ModelEntity::GOLEM, animation) /
                (actions.getDuration(action.getType()) * 1000));
            ...
            player.second->setAnimation(AnimationType::ANIM_IDLE);
            player.second->setCurrentFrame(player.second->getCurrentFrame() +
                                           1);
            if (_module->getMaxFrame(ModelEntity::GOLEM,
                                     player.second->getAnimation()) <=
                player.second->getCurrentFrame())
                player.second->setCurrentFrame(0);
        }
        _module->rotateModel(ModelEntity::GOLEM,
                             player.second->getOrientation());
